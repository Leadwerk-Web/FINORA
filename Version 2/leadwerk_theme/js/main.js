...
document.addEventListener('DOMContentLoaded', () => {
    const leadwerkThemeData = window.leadwerkThemeData || {};
    const defaultLang = String(leadwerkThemeData.defaultLang || 'de').toLowerCase();
    const locale = window.location.pathname.replace(/\\/g, '/').includes('/en/')
        || String(leadwerkThemeData.locale || '').toLowerCase().startsWith('en')
    ...
    const t = copy[locale];
    const wpformsFallback = locale === 'en'
        ? {
            nameLabel: 'Name',
    ...
    const wpformsCopy = Object.assign(
        {},
        wpformsFallback,
        !isDefaultLang && leadwerkThemeData.wpformsTranslations && typeof leadwerkThemeData.wpformsTranslations === 'object'
            ? leadwerkThemeData.wpformsTranslations
            : {}
    );
    ...
    var TESTIMONIAL_COLLAPSED_MAX_HEIGHT = '11.9em';
    ...
    function setTestimonialTextStyle(el, expanded) {
        ...
        if (expanded) {
            ...
            s.overflow = 'hidden';
            s.maxHeight = TESTIMONIAL_COLLAPSED_MAX_HEIGHT;
            s.webkitLineClamp = '';
            s.display = 'block';
        }
    }
    ...
    function getTestimonialCollapsedHeight(textEl) {
        if (!textEl) return 0;

        var styles = window.getComputedStyle(textEl);
        var lineHeight = parseFloat(styles.lineHeight);
        if (!isFinite(lineHeight) || lineHeight <= 0) {
            var fontSize = parseFloat(styles.fontSize);
            lineHeight = (isFinite(fontSize) && fontSize > 0) ? fontSize * 1.7 : 27.2;
        }

        return (lineHeight * 7) + 2;
    }

    function testimonialNeedsToggle(card) {
        if (!card) return false;
        var textEl = card.querySelector('.testimonial-text');
        if (!textEl) return false;

        return textEl.scrollHeight > getTestimonialCollapsedHeight(textEl);
    }
    ...
    function syncTestimonialCard(card, expanded) {
        ...
        var canToggle = !isForcedExpandedTestimonial(card) && testimonialNeedsToggle(card);
        var shouldExpand = isForcedExpandedTestimonial(card) || !canToggle || expanded;
    ...
    // =========================================================
    // Tech Expats: compact system diagram dialog
    // =========================================================
    ...
    if (techDiagramOpen && techDiagramDialog && techDiagramDialogBody && techDiagramSource) {
        ...
        techDiagramDialog.addEventListener('click', function (event) {
            if (event.target === techDiagramDialog) {
                closeTechDiagram();
            }
        });
    ...
    // =========================================================
    if (document.body.hasAttribute('data-leadwerk-back-to-top')) {
