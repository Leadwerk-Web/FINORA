...
class Leadwerk_WPML_Clone {
	...
	protected static function render_editor_page() {
		...
		if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['leadwerk_translation_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['leadwerk_translation_nonce'] ), 'leadwerk_save_translation' ) ) {
			...
			if ( 'save_publish' === $action ) {
				if ( ! empty( $identity_result['publish_error'] ) ) {
				...
				} elseif ( Leadwerk_Translation_Sync::can_publish_bundle( $bundle ) ) {
					wp_update_post(
						array(
							'ID'          => $translated_id,
							'post_status' => 'publish',
					...
					$notice_type = 'error';
					$notice_text = 'Complete every required segment before publishing this translation. Current status: ' . self::status_label( $bundle_state ) . '.';
				}
		...
		foreach ( $sections as $index => $section ) {
			...
			foreach ( $section['segments'] as $segment ) {
				...
				echo '<textarea class="large-text leadwerk-translation-target" rows="' . esc_attr( (string) $target_rows ) . '" name="leadwerk_segments[' . esc_attr( $segment['id'] ) . ']">' . esc_textarea( $segment['translation'] ) . '</textarea>';
	...
	public static function render_settings_page() {
		if ( isset( $_POST['leadwerk_translation_settings_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['leadwerk_translation_settings_nonce'] ), 'leadwerk_save_translation_settings' ) ) {
			Leadwerk_Translation_API::update_settings(
				array(
					'translatable_post_types' => array_values( array_map( 'sanitize_key', (array) ( $_POST['leadwerk_post_types'] ?? array() ) ) ),
	...
	protected static function render_editor_page_settings_card( $settings ) {
		...
		echo '<label class="leadwerk-page-settings-field"><span class="leadwerk-page-settings-label">English Slug</span><textarea class="large-text leadwerk-page-settings-textarea leadwerk-page-settings-target-slug" rows="2" name="leadwerk_page_public_slug" placeholder="/en/page-slug"' . disabled( ! empty( $settings['target_is_home'] ), true, false ) . '>' . esc_textarea( (string) ( $settings['target_slug'] ?? '' ) ) . '</textarea></label>';
	...
	protected static function save_editor_page_settings( $translated_id, $submitted ) {
		...
		$slug_input    = sanitize_title( trim( (string) wp_unslash( $submitted['leadwerk_page_public_slug'] ?? '' ), '/' ) );
...
.leadwerk-bulk-status{font-size:12px;color:#667085;min-height:18px}
