...
class Leadwerk_Translation_API {
	...
	public static function get_default_settings() {
		return array(
			...
			'translatable_post_types' => array( 'page', 'attachment' ),
	...
	public static function update_settings( $settings ) {
		...
		update_option( self::OPTION_SETTINGS, array_merge( self::get_settings(), (array) $settings ) );
		self::$settings_cache = null;
	}
	...
	public static function get_record_by_public_slug( $language_code, $public_slug, $type_like = 'post_%' ) {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM " . self::get_elements_table_name() . " WHERE language_code = %s AND public_slug = %s AND element_type LIKE %s LIMIT 1",
				sanitize_key( (string) $language_code ),
	...
	public static function link_posts( $source_post_id, $translated_post_id, $target_lang, $translated_is_root = false, $args = array() ) {
		...
		$source_record = self::ensure_post_record(
			...
			array(
				...
				'is_home'       => ! empty( $args['source_is_home'] ) || 'finora-home-v1' === get_post_meta( $source_post->ID, 'leadwerk_source_key', true ) || (int) get_option( 'page_on_front' ) === (int) $source_post->ID,
				'status'        => 'complete',
			)
		);
	 ...
	 * @return int
	 */
	public static function get_post_by_source_key( $source_key, $lang = 'de', $preferred_post_type = '' ) {
	...
	protected static function find_translation_candidates_for_source( $source_post, $lang, $source_key, $preferred_target_id = 0, $registered_target_id = 0 ) {
		if ( ! $source_post instanceof WP_Post || '' === $source_key || '' === $lang ) {
			return array();
		}
	...
	protected static function is_post_translation_candidate( $post_id, $source_post, $lang, $source_key ) {
		...
		return $source_key === ( $record_source_key ?: $meta_source_key ) && $lang === ( $record_lang ?: $meta_lang );
	}
	...
	/**
	 * Generate a unique public slug within one language.
	 *
	...
	/**
	 * Whether a public slug is already used by another post in the same language.
	 *
	 * @param string $public_slug Public slug.
	 * @param string $lang        Language code.
	 * @param int    $post_id     Current post ID.
	...
	protected static function public_slug_exists_for_other_post( $public_slug, $lang, $post_id = 0 ) {
		...
		$existing_id = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT element_id FROM " . self::get_elements_table_name() . " WHERE language_code = %s AND public_slug = %s AND element_type LIKE 'post_%%' LIMIT 1",
				sanitize_key( (string) $lang ),
				trim( (string) $public_slug, '/' )
			)
		);

		return $existing_id > 0 && $existing_id !== (int) $post_id;
	}
