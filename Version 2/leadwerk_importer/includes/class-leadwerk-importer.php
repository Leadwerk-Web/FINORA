<?php
/**
 * Main importer for FINORA pages, media and translations.
 *
...
class Leadwerk_Importer {
	...
	public function repair_page_by_source_key( $source_key ) {
		...
		try {
			...
			$job_state = $this->complete_job( $job_state );
	...
	protected function fill_options() {
		...
		foreach ( $option_map as $field_name => $source_path ) {
			...
			if ( $attachment_id && ! get_field( $field_name, 'option' ) ) {
				update_field( $field_name, $attachment_id, 'option' );
			}
		}

		if ( ! get_field( 'company_address', 'option' ) ) {
			update_field( 'company_address', "Brauerstr. 17\n76137 Karlsruhe", 'option' );
		}

		if ( ! get_field( 'company_phone', 'option' ) ) {
			update_field( 'company_phone', '01512 8915214', 'option' );
		}

		if ( ! get_field( 'company_email', 'option' ) ) {
			update_field( 'company_email', 'hallo@finora.de', 'option' );
		}

		if ( ! get_field( 'google_maps_url', 'option' ) ) {
			update_field( 'google_maps_url', 'https://www.google.com/maps/dir/?api=1&destination=Brauerstr.%2017%2C%2076137%20Karlsruhe%2C%20Deutschland', 'option' );
		}

		if ( ! get_field( 'wpforms_form_id_de', 'option' ) ) {
			update_field( 'wpforms_form_id_de', '', 'option' );
		}

		if ( ! get_field( 'wpforms_form_id_en', 'option' ) ) {
			update_field( 'wpforms_form_id_en', '', 'option' );
		}

		if ( ! get_field( 'theme_strings_de', 'option' ) ) {
			update_field( 'theme_strings_de', wp_json_encode( $this->get_default_theme_strings( 'de' ), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ), 'option' );
		}

		if ( ! get_field( 'theme_strings_en', 'option' ) ) {
			update_field( 'theme_strings_en', wp_json_encode( $this->get_default_theme_strings( 'en' ), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ), 'option' );
		}
	}
	...
	protected function get_default_block_content() {
		return '<!-- wp:acf/leadwerk-finora-page /-->';
	}
