...
define( 'LEADWERK_IMPORTER_VERSION', '1.1.1' );
...
$translation_plugin_path = dirname( LEADWERK_IMPORTER_PATH ) . '/leadwerk_wpml_clone';
if ( ! class_exists( 'Leadwerk_Translation_API' ) && is_dir( $translation_plugin_path ) ) {
...
function leadwerk_importer_admin_page() {
	...
	$not_found_source_key = 'finora-404-v1';
	$state = Leadwerk_Logger::get_state();
	...
	if ( '' !== $repair_source_key && wp_verify_nonce( $_GET['_wpnonce'] ?? '', 'leadwerk_repair_page_' . $repair_source_key ) ) {
		$importer     = new Leadwerk_Importer( true );
	...
	<div class="wrap leadwerk-importer-admin">
		...
		<p>FINORA Inhalte, Medien, Field-Gruppen und DE/EN Uebersetzungsstruktur importieren. Der Live-Import laeuft schrittweise, zeigt Fortschritt an und verhindert leere Seiten so gut wie moeglich.</p>
		...
		<div class="leadwerk-importer-toolbar">
			...
			<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'repair_source_key' => 'finora-investment-v1' ), admin_url( 'admin.php?page=leadwerk-import' ) ), 'leadwerk_repair_page_finora-investment-v1' ) ); ?>" class="button">investment-beratung reparieren</a>
